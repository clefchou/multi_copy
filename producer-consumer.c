...
for(;;){
	if((n = read(pfd[0],&tempdataptr,sizeof(struct data *))) < 0){  //这里是如何修改指针的，tempdataptr已经被初始化了怎么就能改变指向的位置？？
//相当于函数内是*struct *&抵消 代表的就是指针
...
for(;;){
	...
	if((n = pread(infd,tempdataptr->buf,BLOCKSIZE,BLOCKSIZE*thr_num)) < 0)
		...
		tempdataptr->ilen = n;
		write(pfd[1],&tempdataptr,sizeof(struct data*));	//管道自带锁，而且传的是指针而不是实际值，因为进程共享堆
		thr_num += NTHREAD;
	}
...
if(outfd < 0){
	printf("creat outfile.txt failed!\n");
	return -1;
}
